...
def patient_generator(env,arrival_rate, triage_levels, treatment_times, doctors, doctor_resource):
    ...
    while True:
        ...
        output_text.insert(END, f"Patient {i} arrived at hospital at: {int(env.now)} minutes.\n")
        ...
        if patient.triage_level == 'emergency':
            with emergency_room.request() as req:
                ...
                with doctor_resource.request(priority=0) as doctor_req:
                    ...
                    output_text.insert(END, f"Patient {patient.id} having emergency got doctor at hospital at: {int(env.now)} minutes.\n")
            ...
            with urgent_room.request() as req:
                ...
                with doctor_resource.request(priority=1) as doctor_req:
                    yield doctor_req
                    output_text.insert(END, f"Patient {patient.id} having urgent case got doctor at hospital at: {int(env.now)} minutes.\n")
            ...
            with urgent_room.request() as req:
                ...
                with doctor_resource.request(priority=1) as doctor_req:
                    yield doctor_req
                    output_text.insert(END, f"Patient {patient.id} not having emergency neither urgency got doctor at hospital at: {int(env.now)} minutes.\n")
                    yield env.timeout(TREATMENT_TIMES[patient.triage_level])
                treatment_end_time = env.now
                patient.treatment_time = treatment_end_time - treatment_start_time
                patient.total_time = patient.wait_time + patient.triage_time + patient.treatment_time
...
def click():
    ...
    image1 = Image.open("C:/Users/piyus/Pictures/Screenshots/Patients Info.png")
...
image = Image.open("C:/Users/piyus/Pictures/Screenshots/Hospital Reception.png")
